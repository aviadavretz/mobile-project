...
class GroceryListTableViewController : UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if (segue.identifier == "ListDetails") {
            ...
            let list:GroceryList = db.lists[selectedRow]
            ...
            list.addRequest(request: item1)
            list.addRequest(request: item2)
            list.addRequest(request: item3)
    ...
    func loadSampleItems() {
        // Add some students
        let list0 = GroceryList(title: "This is my list")
        list0.id = "0"
        
        let list1 = GroceryList(title: "Another list!")
        list1.id = "1"
        
        db.addList(list: list0)
        db.addList(list: list1)
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let list = db.lists[indexPath.row]
